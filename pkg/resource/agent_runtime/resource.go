...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Status.AgentRuntimeID = &identifier.NameOrID

	f1, f1ok := identifier.AdditionalKeys["agentRuntimeVersion"]
	if f1ok {
		r.ko.Status.AgentRuntimeVersion = aws.String(f1)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	f0, ok := fields["agentRuntimeID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: agentRuntimeID"))
	}
	r.ko.Status.AgentRuntimeID = &f0

	f1, f1ok := fields["agentRuntimeVersion"]
	if f1ok {
		r.ko.Status.AgentRuntimeVersion = aws.String(f1)
	}
